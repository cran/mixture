...
void main_loop(int *N, int *p, int *G, double *z, double *sigmar, double *invsigmar, double *mu, double *pi, int *nmax, double *atol, double *mtol, int *mmax, double *x, int *labels, char **covtype, double *logl, int *counter, int *MAPP, double *D, int *MAPPold, double *maps){
    int g, i, n;
    int NN = *N;
    int pp = *p;
    int GG = *G;
    ...
    MAP(z1, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPPold);
    ...
    MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPP);
    maps[1] = comparemap (MAPP, MAPPold, NN);
    MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPPold);
    ...
    MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPP);
     maps[2] = comparemap (MAPP, MAPPold, NN);
     MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPPold);
    ...
    while(getall(logl, i) > aatol && i<(nnmax-1)) {
	 ...
	 MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPP);
	maps[i] = comparemap(MAPP, MAPPold, NN);
       MAP(z, NN, GG, labels, x, pp, mu, Sigma, invSigma, logdet,  MAPPold);
...
double comparemap (int *map1, int *map2, int N) {
...
/*void combinewk (double *z, int N, int G, int *labels)
{
     ...
     for(n=0; n<sum; n++)
      ...
      void combinewk( double *w, int N, int G, int *label){
          int i,j,g;
      double *yy  = malloc(sizeof(double)*N*G);
     for(i=0;i<N;i++)
        for(j=0;j<G;j++)
           yy[i*G+j]= w[i+j*N];
